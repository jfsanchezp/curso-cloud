...
CREATE OR REPLACE FORCE VIEW FORMACION_TICARUM.VW_RESERVA_VUELO_VIGENTE_PASAJERO (ID, ID_VUELO, TIPO_DOCUMENTO_PASAJERO, NUMERO_DOCUMENTO_PASAJERO) AS (
    SELECT RV.ID, RV.ID_VUELO, RVP.TIPO_DOCUMENTO, RVP.NUMERO_DOCUMENTO FROM FORMACION_TICARUM.MUCHOVUELO_RESERVA_VUELO RV
    INNER JOIN FORMACION_TICARUM.MUCHOVUELO_RESERVA_VUELO_PASAJERO RVP
        ON RV.ID = RVP.ID_RESERVA_VUELO
    WHERE RV.ESTADO_RESERVA = 'A' 
    UNION ALL
